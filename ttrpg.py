...
def judgement(pl_abilities: int = 0, gm_abilities: int = 0) -> dict:
    ...
    while True:
        ...
        if outcome != "同値":
            return {"rolls": rolls, "result": outcome}
        # 振り直し: 双方 1d6（能力剥奪）
        reroll_idx += 1
...
def damage_roll(weapon: bool = False, armor: bool = False) -> dict:
    """度合判定。武器+1 / 防具-1 を適用、1〜6 にクリップして重症度に変換。"""
    raw = random.randint(1, 6)
    modifier = (1 if weapon else 0) - (1 if armor else 0)
...
def apply_damage(state: State, severity: Severity, knockout: bool = False) -> dict:
    ...
    if severity == "瀕死":
        ...
        if state == "健常":
            new = "軽傷"
        # 既に軽傷以上の場合、軽傷追加では悪化させない（ルールに記載なし、GM 裁量）
...
def combat_attack(
    ...
    dmg = damage_roll(weapon=weapon, armor=armor)
...
def _cli():
    ...
    a.add_argument("--state", default="健常")
    a.add_argument("--knockout", action="store_true")

    s = sub.add_parser("apply-damage")
    s.add_argument("--state", required=True)
    s.add_argument("--severity", required=True, choices=["軽傷", "重症", "瀕死"])
    ...
    if args.cmd == "judge":
        ...
        out = damage_roll(args.weapon, args.armor)
    elif args.cmd == "attack":
        out = combat_attack(args.pl, args.gm, args.weapon, args.armor, args.state, args.knockout)
